...
public class SqlManager : ISQLManager
{
    ...
    public async Task<ulong> GetLatestTableboundIdentifier(ulong min, ulong max)
    {
        string cmd = "SELECT TABLEBOUNDID FROM TABLEBOUND_PROFILE WHERE TABLEBOUNDID >= $1 AND TABLEBOUNDID < $2 ORDER BY TABLEBOUNDID DESC;";
        var connection = CreateConnection();
        await connection.OpenAsync();
        var command = new NpgsqlCommand(cmd, connection);
        command.Parameters.Add(new NpgsqlParameter() { Value = (long) min });
        command.Parameters.Add(new NpgsqlParameter() { Value = (long) max });
        var reader = await command.ExecuteReaderAsync();
        await reader.ReadAsync();
        if (reader.HasRows)
        {
            Console.WriteLine("Testing this out");
            //We just need to get the top row
            var tableboundid = reader.GetInt64(0);
            await reader.DisposeAsync();
            await connection.CloseAsync();
            Console.WriteLine("Tablebound ID at top was: " + tableboundid);
            return (ulong) tableboundid;
        }
        await reader.DisposeAsync();
        await connection.CloseAsync();
        return 0;
    ...
    #region Username Funcs

    public async Task<List<string>> GetAllUsernames()
    {
        List<string> usernames = new List<string>();
        string cmd = "SELECT USERNAME FROM TABLEBOUND_PROFILE;";
        var connection = CreateConnection();
        await connection.OpenAsync();
        var command = new NpgsqlCommand(cmd, connection);
        var reader = await command.ExecuteReaderAsync();
        while (await reader.ReadAsync()) usernames.Add(reader.GetString(0));
        await reader.DisposeAsync();
        await connection.CloseAsync();
        return usernames;
    }

    public async Task<List<string>> GetUsernameBlacklist()
    {
        List<string> usernames = new List<string>();
        string cmd = "SELECT USERNAME FROM blacklisted_usernames;";
        var connection = CreateConnection();
        await connection.OpenAsync();
        var command = new NpgsqlCommand(cmd, connection);
        var reader = await command.ExecuteReaderAsync();
        Console.WriteLine("read in: " + reader.Rows + " blacklisted usernames.");
        while (await reader.ReadAsync())
        {
    ...
    public async Task UpdateUsername(ulong tableboundIdentifier, string username)
    {
        var cmd = "UPDATE TABLEBOUND_PROFILE SET USERNAME = $1 WHERE TABLEBOUNDID = $2";
        var connection = CreateConnection();
        await connection.OpenAsync();
        var command = new NpgsqlCommand(cmd, connection);
        command.Parameters.Add(new NpgsqlParameter() { Value = username });
        command.Parameters.Add(new NpgsqlParameter() { Value = (long) tableboundIdentifier });
        await command.ExecuteNonQueryAsync();
        await connection.CloseAsync();
    }
